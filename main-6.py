...
voting_classifier_model = load_model('voting_clf-2.pkl')
xgboost_SMOTE_model = load_model('xgboost-smote-2.pkl')
xgboost_featureEngineered_model = load_model('xgboost-featureEngineer-2.pkl')

# Preparing input data function
...
def make_predictions(input_df, input_dict):
    probabilities = {
        ...
        # 'SVM': svm_model.predict(input_df)[0],
        'K-Nearest Neighbors': knn_model.predict_proba(input_df)[0][1]
        # 'Voting Classifier': voting_classifier_model.predict(input_df)[0],
        # 'XGBoost SMOTE': xgboost-smote.predict_proba(input_df)[0][1],
...
# Create a list of customers with their ID and surname
customers = [f"{row['CustomerId']} - {row['Surname']}" for _, row in df.iterrows()]

# Select box for customer selection
...
# Prepare the input and make predictions
